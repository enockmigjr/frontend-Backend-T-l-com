...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (accessToken) upstream = await backendJson('/api/v1/auth/logout-all', 'POST', undefined, accessToken);
    if (!upstream || upstream.status === 401) {
      if (!refreshToken)
        return cleared(new NextResponse(null, { status: 204, headers: { 'Cache-Control': 'no-store' } }));
      ...
      upstream = await backendJson('/api/v1/auth/logout-all', 'POST', undefined, accessToken);
    }
