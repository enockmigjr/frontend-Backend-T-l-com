...
async function transition(page: Page, button: string, action: string): Promise<void> {
  const responsePromise = page.waitForResponse(
    (response) => new URL(response.url()).pathname.endsWith(`/${action}`) && response.request().method() === 'POST',
  );
...
test('@critical-4 exécute le cycle ticket complet et livre une notification', async ({ browser, page }, testInfo) => {
  ...
  await page
    ...
    .fill('Incident créé par Playwright pour valider le parcours réel complet.');
  ...
  const agentInfo = page.getByText('Agent assigné', { exact: true }).locator('..').locator('dd');
  ...
  await page.getByPlaceholder('Rédiger une réponse visible par les équipes…').fill(comment);
  ...
  await page.getByPlaceholder('Ajouter une note privée aux équipes internes…').fill(note);
  const noteResponse = page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname.endsWith('/internal-notes') && response.request().method() === 'POST',
  );
  await page.getByRole('button', { name: 'Publier' }).click();
  ...
  const deleteResponse = page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname === `/api/v1/tickets/${ticketId}` && response.request().method() === 'DELETE',
  );
